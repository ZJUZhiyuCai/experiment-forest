...
export function Sidebar({ isCollapsed, onToggle }: SidebarProps) {
  ...
  return (
    <motion.div
      className={cn(
         'fixed h-full bg-white dark:bg-gradient-to-b dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-r border-[#E6F0FA] dark:border-slate-700 z-30 transition-all duration-300 ease-in-out',
         isCollapsed ? 'w-16' : 'w-64',
         'shadow-sm rounded-r-2xl dark:shadow-2xl'
       )}
    >
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-600">
        <div className={cn('flex items-center', !isCollapsed && 'space-x-2')}>
          <i className="fa-solid fa-flask text-emerald-600 dark:text-emerald-400 text-xl"></i>
          {!isCollapsed && (
            <div>
              <h1 className="text-xl font-bold text-gray-800 dark:text-slate-100">实验小森林</h1>
              <p className="text-xs text-gray-500 dark:text-slate-400">记录每一次科学的萌芽</p>
            </div>
          )}
        </div>
        <button 
        ...
        >
          <i className={`fa-solid ${isCollapsed ? 'fa-chevron-right' : 'fa-chevron-left'} text-gray-600 dark:text-slate-300`}></i>
        </button>
      </div>
  ...
  {navItems.map((item) => (
    <li key={item.path} className="mb-1">
      ...
      <NavLink
        to={item.path}
        className={cn(
          'flex items-center px-3 py-3 rounded-lg transition-all duration-200 cursor-pointer w-full',
          isActivePath(item.path) 
            ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 font-medium' 
            : 'text-gray-600 dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-emerald-800/50'
        )}
        onClick={(e) => {
