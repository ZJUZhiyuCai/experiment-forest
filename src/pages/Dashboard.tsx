      ...
      <div className={sidebarCollapsed ? 'ml-16' : 'ml-64'}>
        ...
        <main className="container mx-auto px-4 py-6">
          ...
          <section className="mb-8">
            <div className="bg-gradient-to-br from-forest-accent/40 to-forest-secondary/20 rounded-2xl border border-forest-accent/50 p-8 shadow-nature">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-header font-bold text-forest-primary mb-2 flex items-center">
                    <i className="fa-solid fa-seedling mr-3 text-forest-secondary"></i>
                    欢迎回到实验小森林
                  </h1>
                  <p className="text-text-soft text-lg">今日新增 {recentRecords.filter(r => new Date(r.date).toDateString() === new Date().toDateString()).length} 条记录，森林正在茁壮成长 🌱</p>
                </div>
                <div className="text-right bg-white/50 backdrop-blur-sm p-4 rounded-xl border border-white/50">
                  <p className="text-xs text-text-muted uppercase tracking-wider font-bold">森林繁茂度</p>
                  <p className="text-3xl font-header font-bold text-forest-primary">{Math.min(100, (stats.records + stats.notes + stats.sops) * 2)}%</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <motion.div
              className="bg-gradient-to-br from-forest-secondary to-forest-primary rounded-2xl shadow-nature p-6 text-white flex flex-col justify-between"
              whileHover={{ scale: 1.02, translateY: -4 }}
              transition={{ duration: 0.3 }}
            >
              <div>
                <h3 className="text-xl font-header mb-1 flex items-center">
                  <i className="fa-solid fa-seedling mr-2"></i>
                  实验记录
                </h3>
                <p className="text-white/80 text-sm mb-4">种下实验的种子，记录科学的萌芽</p>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-3xl font-header">{stats.records}</span>
                <Link to="/records/new" className="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-lg transition-all border border-white/30 text-sm font-bold">
            ...
            <motion.div
              className="bg-gradient-to-br from-forest-primary to-earth-brown rounded-2xl shadow-nature p-6 text-white flex flex-col justify-between"
              whileHover={{ scale: 1.02, translateY: -4 }}
              transition={{ duration: 0.3 }}
            >
              <div>
                ...
                <p className="text-white/80 text-sm mb-4">为知识之树增添新的枝叶</p>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-3xl font-header">{stats.notes}</span>
                <Link to="/notes/new" className="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-lg transition-all border border-white/30 text-sm font-bold">
                  <i className="fa-solid fa-plus mr-1"></i>
                  生长
                </Link>
              </div>
            ...
            <motion.div
              className="bg-gradient-to-br from-earth-brown to-earth-brown rounded-2xl shadow-nature p-6 text-white flex flex-col justify-between"
              whileHover={{ scale: 1.02, translateY: -4 }}
              transition={{ duration: 0.3 }}
            >
              <div>
                ...
                <p className="text-white/80 text-sm mb-4">培育成熟的知识大树</p>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-3xl font-header">{stats.sops}</span>
                <Link to="/sops/new" className="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-lg transition-all border border-white/30 text-sm font-bold">
                  <i className="fa-solid fa-plus mr-1"></i>
                  栽培
                </Link>
              </div>
          ...
          <section className="mb-10">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-header font-bold text-forest-primary flex items-center">
                <i className="fa-solid fa-seedling mr-2 text-forest-secondary"></i>
                最近的幼苗
              </h2>
          ...
          <section className="mb-10">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-forest-primary flex items-center">
                <i className="fa-solid fa-leaf mr-2 text-forest-secondary"></i>
                最新的枝叶
              </h2>
              <Button asChild variant="outline" size="sm">
          ...
          <section>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-forest-primary flex items-center">
                <i className="fa-solid fa-tree mr-2 text-forest-secondary"></i>
                知识大树
              </h2>
              <Button asChild variant="outline" size="sm">
          ...
          <section className="mb-10">
            ...
            <div className="bg-white dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-600 p-6 transition-all duration-300 hover:shadow-md dark:hover:shadow-2xl">
                  ...
                  return (
                    <div
                      ...
                      className={`
                         ...
                         ${hasRecord ? 'bg-forest-accent/20 text-text-main dark:bg-emerald-900/30 dark:text-emerald-200 font-medium' : 'text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700'}
                         ${isToday ? 'ring-2 ring-forest-accent' : ''}
                       `}
                      title={`${recordCount} 条实验记录`}
                    >
                      <span>{date}</span>
                      {hasRecord && (
                        <span className="ml-1 w-1.5 h-1.5 rounded-full bg-forest-accent"></span>
                      )}
                    </div>
                  );
                })}
              ...
              <div className="mt-4 flex justify-center">
                <Button asChild size="sm" className="bg-forest-accent hover:bg-forest-secondary">
                  <Link to="/calendar">
